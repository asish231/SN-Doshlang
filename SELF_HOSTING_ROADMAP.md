...
`snc` is already a substantial real compiler: `src/parser.s`, `src/codegen.s`, `src/data.s`, `src/vars.s`, `src/utils.s`, `src/lexer.s`, and `src/main.s` total **24,459 lines** of hand-written ARM64 assembly (`src/parser.s` alone is `14,950` lines). Per the verified current state, it builds cleanly, passes `make assert` (`200/200`), and compiles/links/runs `155/162` examples, where the remaining `7` are deliberate negative tests.

That means the main self-hosting blocker is no longer “the language is too small.” The verified feature set already covers the essentials for a compiler implementation: integers, control flow, functions and recursion, strings and interpolation, lists, file I/O, `try/catch/throw`, nested `fn` definitions, and runtime `for-in` over lists/params. The driver plumbing (`argc`/`argv`, `exec`/`system`), the larger compiler-internal tables, and a `1MB` source buffer have now been **implemented** (see the prerequisites table). The remaining blockers are **growable runtime collections** (the emitted `list`/`map` pools), **compiler-grade string building**, and **backend factoring**.
...
| Growable compiler-internal tables | The compiler source will exceed today’s hard caps quickly | The **operation and print/data tables are now `malloc`-backed and grow on demand** — `_snc_grow_ops`/`_snc_grow_prints` (`src/vars.s`) realloc-double them; a 60k-op and a 20k-print program compile+link+run. The variable/function tables are still large-but-static (`platform.inc` caps: vars `4096`, functions `256`) | **Partly done (op/print dynamic; vars/fns static)** |
...
| Growable runtime collections | The compiler itself needs symbol tables, intern tables, token vectors, and string-keyed maps | Current lists/maps live in fixed pools: `list_pool_*` at `32768`, `map_pool_*` at `32768`, and tiny `dyn_map_*` runtime tables at `8192` in `asm_map_dynamic_runtime` | **Not started** |
| Robust string building | Codegen must emit large `.s` files without quadratic `+` concatenation behavior | Strings and interpolation are verified, but there is no compiler-grade append buffer API yet | **Not started** |
...
| Long-run memory discipline | Leak-by-design is fine for tiny CLI programs, risky for a compiler | The current runtime intentionally `malloc`s and never `free`s | **Not started** |
| Source-size headroom | The compiler should not only work if it barely fits one static source buffer | Source buffer raised from `64KB` to `1MB` (`SNC_SRC_BYTES`); the read cap now derives from `src_buffer_cap` | **Done** |

#### Immediate interpretation of that checklist

- Enough **language** exists to write a compiler now.
- Enough **compiler-scale infrastructure** does not yet exist.
- Therefore the next work should be infrastructure first, feature growth second.
...
| `M1` — Finish the immediate prerequisites | `M0` frozen | Land SNlang-level `argc`/`argv`; land `exec`/`system` or equivalent; land fixed-table growth; if needed, keep compiler source multi-module to avoid the `65535`-byte input cap | A SNlang driver can accept flags, emit `.s`, invoke the toolchain, and no longer falls over at realistic compiler sizes |
| `M2` — Build a tiny compiler runtime in SNlang | `M1` available | Add a phase arena, growable vectors, a string-keyed map/dict, and a buffered string builder/output writer; keep this compiler-specific instead of trying to grow the general `stdlib` first | The runtime can hold thousands of tokens/IR entries and emit large outputs without quadratic string behavior |
...
| Leak-by-design memory | The current runtime’s “`malloc` but never `free`” model is fine for short CLI examples, but risky for a self-hosting compiler | Introduce a phase arena in `M2`; reclaim by phase or by process, not object-by-object; if necessary keep early self-host runs one-file-per-process |
