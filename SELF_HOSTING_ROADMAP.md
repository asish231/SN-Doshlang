...
- The **fixed-cap blockers are gone**: every compiler table, the `list`/`map` runtime pools, and the source buffer now grow dynamically. What remains on the infrastructure side is a compiler-grade **string/output builder** and a **long-run memory story** (arena / `free`), plus the SNlang-level runtime API in M2.
